...
std::vector<ref<BitExpr>> getBitExpr(ref<Expr> expr,
                                     const std::vector<unsigned> &bit_offsets) {
  ...
  case Expr::Concat: {
    ...
    std::vector<ref<BitExpr>> result;
    for (auto i = 0u, left_i = 0u, right_i = 0u; i < bit_offsets.size(); i++) {
      if (is_right[i]) {
  ...
  case Expr::Eq: {
    ...
    for (auto i = 0u; i < not_result.size(); i++) {
      if (auto constant_result = dyn_cast<ConstantBitExpr>(not_result.at(i))) {
        ...
        result = BinaryBitExpr::create(BitExpr::And, result, not_result.at(i));
      }
    }
